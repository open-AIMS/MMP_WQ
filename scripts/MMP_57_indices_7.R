...
if ((alwaysExtract | !file.exists(paste0(INDICES_OUTPUT_PATH,"wq.alt6.idx.RData"))) &
    ...
    MMP_add_to_report_list(CURRENT_STAGE, "calculate indices",
                           SUBSECTION_7 = structure(paste0("## Type 7\n"),
                                                    parent = 'TABSET'),
    ...
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Reading in data for alt7 indices.', return=TRUE)
    ## ----end
  ...
  MMP_tryCatch(
  {
    wq.alt7.idx <-
      (wq.alt7.qaqc1 <-
         (wq.alt7.qaqc <-                               # Retain compiled data for QAQC 1
  ...
  LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Calculate alt7 indices.', return=TRUE)
  ## ----end
  ...
  {
    ##QAQC figure
    p <- mmp__qaqc(wq.alt7.qaqc, level = 1, type = '7',
                   wq.units = wq.units,
                   wq.sites = wq.sites
                   )

    MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_qaqc",
                           Plot = p, units = "in",
                           fig.width = 180/25.4, fig.height = 180/25.4, pt.size = 10)

    MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                       Section = "QAQC", fig_name_suffix = "wq_alt7_qaqc",
                       label_suffix = "_7_qaqc", tabset_parent = "TABSET_7",
                       fig.caption = paste0("\nObserved ",as.numeric(reportYear),"/",as.numeric(reportYear)," water quality data associated with the alt7 (formulation 7) indices. Red and blue symbols represent dry and wet season samples. The purple band defines half and twice the annual guideline values.\n")) 
    
  },
  LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 QAQC plot 1.', return=TRUE)
  ## ----end
  ...
  {
    ##QAQC figure 1
    p <- mmp__qaqc(wq.alt7.qaqc1, level = 2, type = '7',
                   wq.units = wq.units,
                   wq.sites = wq.sites
                   )

    MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_qaqc1",
                           Plot = p, units = "in",
                           fig.width = 180/25.4, fig.height = 180/25.4, pt.size = 10)

    MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                       Section = "QAQC 1", fig_name_suffix = "wq_alt7_qaqc1",
                       label_suffix = "_7_qaqc1", tabset_parent = "TABSET_7",
                       fig.caption = paste0("\nSeasonal or annual ",as.numeric(reportYear),"/",as.numeric(reportYear)," water quality averages associated with the alt7 (formulation 7) indices. Red and blue symbols represent dry and wet season samples. The purple band defines half and twice the annual guideline values.\n")) 

  },
  LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 QAQC plot 2.', return=TRUE)
  ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        reneeYear.site.measure.season <- wq.alt7.idx %>%
            mutate(Weight = 1) %>%
        ...
        ##QAQC figure idx
        p <- mmp__qaqc(reneeYear.site.measure, level = 3, type = '7',
                       wq.units = wq.units,
                       wq.sites = wq.sites
                       )

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_qaqc2",
                                    Plot = p, units = "in",
                                    fig.width = 180/25.4, fig.height = 180/25.4, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "QAQC 2", fig_name_suffix = "wq_alt7_qaqc2",
                           label_suffix = "_7_qaqc2", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nSeasonal or annual ",as.numeric(reportYear),"/",as.numeric(reportYear)," water quality averages associated with the alt7 (formulation 7) indices. Red and blue symbols represent dry and wet season samples. The purple band defines half and twice the annual guideline values.\n")) 

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 QAQC plot 3.', return=TRUE)
    ## ----end
    ...
    {
        ## Regional Worms
        p <- mmp__indicator_trends(reneeYear.site.measure, level = 1, type = '7',
                       wq.units = wq.units,
                       wq.sites = wq.sites
                       )

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_site_measure",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Site/Measure Index", fig_name_suffix = "wq_alt7_idx_site_measure",
                           label_suffix = "_7_idx1", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 Index plot 1.', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        ## Worms
        p <- mmp__indicator_trends(reneeYear.subregion.measure, level = 2, type = '7',
                       wq.units = wq.units,
                       wq.sites = wq.sites
                       )

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_subregion_measure",
                               Plot = p  + facet_grid(Subindicator + Measure ~ Subregion,
                                                      as.table = TRUE,
                                                      labeller = labeller(Subregion =
                                                                              label_wrap_gen(10))),
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional/Measure Index", fig_name_suffix = "wq_alt7_idx_subregion_measure",
                           label_suffix = "_7_idx2", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_subregion_measure_CI",
                               Plot = p  + facet_grid(Subindicator + Measure ~ Subregion,
                                                      as.table = TRUE,
                                                      labeller = labeller(Subregion =
                                                                              label_wrap_gen(10))) +
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional/Measure Index Alt.", fig_name_suffix = "wq_alt7_idx_subregion_measure_CI",
                           label_suffix = "_7_idx2a", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 

        reneeYear.subregion.measure.worm <- p$data
    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 Index plot 2.', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        ## Worms
        p <- mmp__indicator_trends(reneeYear.region.measure, level = 3, type = '7',
                                   wq.units = wq.units,
                                   wq.sites = wq.sites
                                   )

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_region_measure",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional/Measure Index", fig_name_suffix = "wq_alt7_idx_region_measure",
                           label_suffix = "_7_idx3", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_region_measure_CI",
                               Plot = p  + facet_grid(Subindicator + Measure ~ Region,
                                                      as.table = TRUE) +
                                   geom_linerange(aes(ymin = Lower, ymax = Upper)),
                               units = "in",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional/Measure Index Alt.", fig_name_suffix = "wq_alt7_idx_region_measure_CI",
                           label_suffix = "_7_idx3a", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 Index plot 3.', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        ## Worms
        p <- mmp__indicator_trends(reneeYear.subregion.subindicator, level = 4, type = '7',
                       wq.units = wq.units,
                       wq.sites = wq.sites
                       )

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_subregion_subindicator",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional/Subindicator Index", fig_name_suffix = "wq_alt7_idx_subregion_subindicator",
                           label_suffix = "_7_idx4", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_subregion_subindicator_CI",
                               Plot = p  + facet_grid(Subindicator ~ Subregion,
                                                      as.table = TRUE,
                                                      labeller = labeller(Subregion =
                                                                              label_wrap_gen(10))) +
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional/Subindicator Index Alt.", fig_name_suffix = "wq_alt7_idx_subregion_subindicator_CI",
                           label_suffix = "_7_idx4a", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 

        wq.alt7.idx.subregion.subindicator <- reneeYear.subregion.subindicator.worm <- p$data
        save(wq.alt7.idx.subregion.subindicator,
             file = paste0(DATA_PATH, '/indices/wq.alt7.idx.subregion.subindicator.RData'))

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 Index plot 4.', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_region_subindicator",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional/Subindicator Index", fig_name_suffix = "wq_alt7_idx_region_subindicator",
                           label_suffix = "_7_idx5", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_region_subindicator_CI",
                               Plot = p  + facet_grid(Subindicator ~ Region,
                                                      as.table = TRUE) +
                                   geom_linerange(aes(ymin = Lower, ymax = Upper)),
                               units = "in",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional/Subindicator Index Alt.", fig_name_suffix = "wq_alt7_idx_region_subindicator_CI",
                           label_suffix = "_7_idx5a", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 

        wq.alt7.idx.region.subindicator <- p$data
        save(wq.alt7.idx.region.subindicator,
             file = paste0(DATA_PATH, '/indices/wq.alt7.idx.region.subindicator.RData'))

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 Index plot 5.', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        ## Worms
        p <- mmp__indicator_trends(reneeYear.subregion.indicator, level = 6, type = '7',
                       wq.units = wq.units,
                       wq.sites = wq.sites
                       )

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_subregion_indicator",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional/Indicator Index", fig_name_suffix = "wq_alt7_idx_subregion_indicator",
                           label_suffix = "_7_idx6", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_subregion_indicator_CI",
                               Plot = p  + facet_grid(Indicator ~ Subregion,
                                                      as.table = TRUE,
                                                      labeller = labeller(Subregion =
                                                                              label_wrap_gen(10))) +
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional/Indicator Index Alt.", fig_name_suffix = "wq_alt7_idx_subregion_indicator_CI",
                           label_suffix = "_7_idx6a", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        wq.alt7.idx.subregion.indicator <- p$data
        save(wq.alt7.idx.subregion.indicator,
             file = paste0(DATA_PATH, '/indices/wq.alt7.idx.subregion.indicator.RData'))
        reneeYear.subregion.indicator.worm <- p$data
        wq.alt7.idx.subregion <- reneeYear.subregion.indicator.worm
        save(wq.alt7.idx.subregion,
             file = paste0(DATA_PATH, '/final/wq.alt7.idx.subregion.RData'))

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Generate alt7 Index plot 7.', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        ## Worms
        p <- mmp__indicator_trends(reneeYear.region.indicator, level = 7, type = '7',
                       wq.units = wq.units,
                       wq.sites = wq.sites
                       )

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_region_indicator",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional/Indicator Index", fig_name_suffix = "wq_alt7_idx_region_indicator",
                           label_suffix = "_7_idx7", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_region_indicator_CI",
                               Plot = p  + facet_grid(Indicator ~ Region,
                                                      as.table = TRUE) +
                                   geom_linerange(aes(ymin = Lower, ymax = Upper)),
                               units = "in",
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional/Indicator Index Alt.", fig_name_suffix = "wq_alt7_idx_region_indicator_CI",
                           label_suffix = "_7_idx7a", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 


        wq.alt7.idx.region.indicator <- p$data
        save(wq.alt7.idx.region.indicator,
             file = paste0(DATA_PATH, '/indices/wq.alt7.idx.region.indicator.RData'))
        reneeYear.region.indicator.worm <- p$data

        wq.alt7.idx.region <- reneeYear.region.indicator.worm
        save(wq.alt7.idx.region,
             file = paste0(DATA_PATH, '/final/wq.alt7.idx.region.RData'))
    ...
    {
        p <-
            ggplot(wq.alt7.idx.region.indicator, aes(y = Index, x = reportCardYear)) +
            geom_hline(yintercept = 0, linetype = 'dashed') +
            geom_linerange(aes(ymin = Lower, ymax = Upper)) +
            geom_line(data = wq.alt7.idx.region.subindicator, aes(color = Subindicator)) +
            geom_line() +
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_alt7_idx_region_and_subindicators",
                               Plot = p  + facet_grid(Indicator ~ Region,
                                                      as.table = TRUE),
                               units = "in",
                               fig.width = 2*180/25.4, fig.height = 1*(2*(180*(2/10))/25.4),
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional/(sub)Indicator Index", fig_name_suffix = "wq_alt7_idx_region_and_subindicators",
                           label_suffix = "_7_idx8", tabset_parent = "TABSET_7",
                           fig.caption = paste0("\nTemporal trends in the water quality index conditional on Region for the alt7 (formulation 7) indices.\n")) 
    ...
    MMP_tryCatch(
    {
        save(reneeYear.site.measure,
             file = paste0(DATA_PATH, '/final/reneeYear.site.measure_7.RData'))
        save(reneeYear.subregion.measure,
             file = paste0(DATA_PATH, '/final/reneeYear.subregion.measure_7.RData'))
        save(reneeYear.region.measure,
             file = paste0(DATA_PATH, '/final/reneeYear.region.measure_7.RData'))
        save(reneeYear.subregion.subindicator,
             file = paste0(DATA_PATH, '/final/reneeYear.subregion.subindicator_7.RData'))
        save(reneeYear.subregion.indicator,
             file = paste0(DATA_PATH, '/final/reneeYear.subregion.indicator_7.RData'))
        save(reneeYear.region.indicator,
             file = paste0(DATA_PATH, '/final/reneeYear.region.indicator_7.RData'))

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Save data for excel (Version 7).', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        alt7.idx.tab1 <- reneeYear.subregion.measure.worm %>%
        ...
        save(alt7.idx.tab1,
             file = paste0(DATA_PATH, '/final/alt7.idx.tab1.RData'))
        
        alt7.idx.tab2 <- reneeYear.subregion.subindicator.worm %>%
            filter(reneeYear==reportYear) %>%
        ...
        save(alt7.idx.tab2,
             file=paste0(DATA_PATH, '/final/alt7.idx.tab2.RData'))
        
        alt7.idx.tab3 <- reneeYear.subregion.indicator.worm %>%
            filter(reneeYear == reportYear) %>%
        ...
        save(alt7.idx.tab3,
             file = paste0(DATA_PATH, '/final/alt7.idx.tab3.RData'))

        save(wq.alt7.idx,
             file = paste0(INDICES_OUTPUT_PATH, 'wq.alt7.idx.RData'))
        
    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Save additional derivatives (Version 7).', return=TRUE)
    ## ----end

  
    MMP_checkData(name = "wq.alt7.idx.RData",
