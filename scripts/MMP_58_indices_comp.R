...
if ((alwaysExtract | !file.exists(paste0(OUTPUT_PATH,"/figures/Plots4Renee.zip"))) &
    ...
    MMP_tryCatch(
    {
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_comb_region",
                               ...
                               units = "in",
                               fig.width = 10, fig.height = 3, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (comp)", fig_name_suffix = "wq_comb_region",
                           label_suffix = "_9_idx1", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 6 formulations conditional on Region. NOTE, this is the old annual index.\n")) 
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index alt. (comp)", fig_name_suffix = "wq_comb_region1",
    ...
    ## 1b. Regional worms - Annual new vs old
    ## These differ in the old vs new hierarchy
    ## ---- Regional worms
    MMP_tryCatch(
    {
        load(file=paste0(DATA_PATH, '/final/wq.alt6.idx.region.RData'))
        ...
        wq.comb.region.g1<-
            ggplot(data = wq.alt6.idx.region %>%
                       ## filter(Year>2007) %>%
                       mutate(Region = factor(Region,
                                              levels=c('Cape York','Wet Tropics',
                                                       'Burdekin','Mackay Whitsunday',
            ...
            geom_hline(yintercept = 0, linetype = 'dashed') +
            geom_line() +
            geom_point(aes(fill = Grade, shape = '6', color = '6'),
                       size = 2) +
            geom_line(data = wq.alt7.idx.region) +
            geom_point(data = wq.alt7.idx.region,
                       aes(fill = Grade, shape = '7', color = '7'),
                       size = 2.5) + 
            scale_y_continuous('Water Quality Index',
                               limits = c(-1,1)) +
            scale_x_date('', limits = c(as.Date('2006-01-01'),
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_comb_region_new_old",
                               Plot = wq.comb.region.g1 + facet_grid(~Region,as.table=FALSE),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (old vs new)", fig_name_suffix = "wq_comb_region_new_old",
                           label_suffix = "_9_idx1b", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for alt 6 (old) and alt 7 (new) formulations conditional on Region. \n")) 

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_comb_region1_new_old",
                               Plot = wq.comb.region.g1 + facet_wrap(~Region,as.table=FALSE,nrow=1,scales='free_y'),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index alt. (old vs new)", fig_name_suffix = "wq_comb_region1_new_old",
                           label_suffix = "_9_idx1ab", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the alt 6 (old) and alt 7 (new) formulations conditional on Region.\n")) 

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Comparing indices (region level 2).', return=TRUE)
    ## ----end

    ## 1c. Regional worms - Long-term new vs old
    ## These differ in index type (AMP vs MAMP) and hierarchy
    ## ---- Regional worms
    MMP_tryCatch(
    {
        load(file=paste0(DATA_PATH, '/final/wq.historic.idx.region.RData'))
        load(file=paste0(DATA_PATH, '/final/wq.alt8.idx.region.RData'))
        ...
        wq.comb.region.g1<-
            ...
            geom_point(aes(fill = Grade, shape = '0', color = '0'),
                       size = 2) +
            geom_line(data = wq.alt8.idx.region) +
            geom_point(data = wq.alt8.idx.region,
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_comb_region_long_new_old",
                               Plot = wq.comb.region.g1 + facet_grid(~Region,as.table=FALSE),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (long old vs new)", fig_name_suffix = "wq_comb_region_long_new_old",
                           label_suffix = "_9_idx1bc", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for alt 0 (old) and alt 8 (new) formulations conditional on Region. \n")) 

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_comb_region1_long_new_old",
                               Plot = wq.comb.region.g1 + facet_wrap(~Region,as.table=FALSE,nrow=1,scales='free_y'),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index alt. (long old vs new)", fig_name_suffix = "wq_comb_region1_long_new_old",
                           label_suffix = "_9_idx1abc", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the alt 0 (old) and alt 8 (new) formulations conditional on Region.\n")) 
    ...
    MMP_tryCatch(
    {
        ...
        g1 <- ggplot(dat, aes(y = Index, x = reportCardYear)) +
            ...
            geom_linerange(data = wq.alt7.idx.region %>%
                               filter(Region != 'Cape York') %>%
                               droplevels,
    ...
    MMP_tryCatch(
    {
        ...
        ## wq.comb.region.g1 <- ggplot(wq.historic.idx.region %>%
        wq.comb.region.g1 <- ggplot(wq.alt8.idx.region %>%
                                    mutate(Year = reneeYear) |> 
                                    filter(Year>2007, Year < 2016) %>%
                                    mutate(Region = factor(Region,
                                                           levels = levels(wq.alt7.idx.region$Region))),
                                    aes(y = Index, x = reportCardYear)) +
            geom_hline(yintercept = 0, linetype = 'dashed') +
            geom_line() +
            geom_point(aes(fill = Grade,shape = '0'),
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_region_new",
                               Plot = wq.comb.region.g1 + facet_grid(~Region,as.table=FALSE),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (new)", fig_name_suffix = "wq_worms_comb_region_new",
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_region1_new",
                               Plot = wq.comb.region.g1 + facet_wrap(~Region,as.table=TRUE,nrow=1,scales='free'),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index alt. (new)", fig_name_suffix = "wq_worms_comb_region1_new",
    ...
    MMP_tryCatch(
    {
        ...
        wq.alt7.idx.region_noCY <- wq.alt7.idx.region %>%
            filter(Region != 'Cape York') %>%
            droplevels()
        ...
        wq.comb.region.g1 <- ggplot(wq.alt8.idx.region %>%
                                    mutate(Year = reneeYear) |> 
                                    filter(Year>2007, Year < 2016, Region != 'Cape York') %>%
                                    droplevels() %>%
                                    mutate(Region = factor(Region,
                                                           levels = levels(wq.alt5.idx.region_noCY$Region))),
                                    aes(y = Index, x = reportCardYear)) +
            geom_hline(yintercept = 0, linetype = 'dashed') +
            geom_line() +
            geom_point(aes(fill = Grade, shape = '0'), size = 2 * point_mult, show.legend = FALSE) +
            geom_linerange(aes(ymin = Lower, ymax = Upper, fill = Grade,
                               shape = '0'),
                           show.legend = FALSE) + 
            geom_line(data = wq.alt7.idx.region_noCY) +
            geom_point(data = wq.alt7.idx.region_noCY,
                       aes(fill = Grade, shape = '7'), size = 2 * point_mult, show.legend = FALSE) +
            geom_linerange(data = wq.alt7.idx.region_noCY,
                           aes(ymin = Lower, ymax = Upper, fill = Grade,
                               shape = '7'),
                           show.legend = FALSE) + 
            scale_y_continuous('Water Quality Index',
            ...
            scale_fill_manual('', breaks = c('A','B','C','D','E'),
                              ...
                              labels=c('Very Good','Good','Moderate','Poor','Very Poor'),
                              guide=FALSE) +
            scale_shape_manual('Option', values = c(21,24,23,22,25)) +
            theme_mmp +
            theme(strip.background = element_blank(),
                  panel.margin.x = unit(1,'line'))

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_region_NoCapeYork_new",
                               Plot = wq.comb.region.g1 + facet_grid(~Region,as.table=FALSE),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (No CY, new)", fig_name_suffix = "wq_worms_comb_region_NoCapeYork_new",
                           label_suffix = "_9_idx4b", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_region1_NoCapeYork_new",
                               Plot = wq.comb.region.g1 + facet_wrap(~Region,as.table=TRUE,nrow=1,scales='free'),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (No CY, new) alt.", fig_name_suffix = "wq_worms_comb_region1_NoCapeYork_new",
                           label_suffix = "_9_idx4ab", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 

    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Comparing indices (No Cape York or Fitzroy, new).', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        ...
        wq.alt7.idx.region_noF <- wq.alt7.idx.region %>%
            filter(Region != 'Fitzroy') %>%
            droplevels()
        point_mult <- 1.5
        ...
        wq.comb.region.g1 <- ggplot(wq.alt8.idx.region %>%
                                    mutate(Year = reneeYear) |> 
                                    filter(Year>2007, Year < 2016, Region != 'Fitzroy') %>%
                                    droplevels() %>%
                                    mutate(Region = factor(Region,
                                                           ## levels = levels(wq.alt6.idx.region_noCY$Region))),
                                                           levels = levels(wq.alt7.idx.region_noF$Region))),
                                    aes(y = Index, x = reportCardYear)) +
            geom_hline(yintercept = 0, linetype = 'dashed') +
            geom_line() +
            geom_point(aes(fill = Grade, shape = '0'), size = 2 * point_mult, show.legend = TRUE) +
            geom_linerange(aes(ymin = Lower, ymax = Upper, fill = Grade,
                               shape = '0'),
                           show.legend = FALSE) + 
            geom_line(data = wq.alt7.idx.region_noF) +
            geom_point(data = wq.alt7.idx.region_noF,
                       aes(fill = Grade, shape = '7'), size = 2 * point_mult, show.legend = TRUE) +
            geom_linerange(data = wq.alt7.idx.region_noF,
                           aes(ymin = Lower, ymax = Upper, fill = Grade,
                               shape = '7'),
                           show.legend = FALSE) + 
            scale_y_continuous('Water Quality Index',
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_region_NoFitzroy_new",
                               Plot = wq.comb.region.g1 + facet_grid(~Region,as.table=FALSE),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (No Ftz, new)", fig_name_suffix = "wq_worms_comb_region_NoFitzroy_new",
                           label_suffix = "_9_idx5b", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_region1_NoFitzroy_new",
                               Plot = wq.comb.region.g1 + facet_wrap(~Region,as.table=TRUE,nrow=1,scales='free'),
                               units = "in",
                               fig.width = 10, fig.height = 2.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Regional Index (No Ftz, new) alt.", fig_name_suffix = "wq_worms_comb_region1_NoFitzroy_new",
                           label_suffix = "_9_idx5ab", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 
    ...
    MMP_tryCatch(
    {
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_subregion",
                               Plot = wq.comb.subregion.g1 + facet_grid(~Subregion,as.table=FALSE),
                               units = "in",
                               fig.width = 10, fig.height = 2, pt.size = 10)
    ...
    MMP_tryCatch(
    {
        ...
        wq.comb.subregion.g1 <-
            ## ggplot(wq.historic.idx.subregion %>%
            ggplot(wq.alt8.idx.subregion %>%
                   mutate(Year = reneeYear) |> 
                   filter(Year>2007, Year < 2016) %>%
                   mutate(Subregion =
                              factor(Subregion,
                                     levels = levels(wq.alt7.idx.subregion$Subregion))),
                   aes(y = Index, x = reportCardYear)) +
            geom_hline(yintercept = 0, linetype = 'dashed') +
            geom_line() +
            geom_point(aes(fill = Grade, shape = '0'), size = 2 * point_mult) +
            geom_linerange(aes(ymin = Lower, ymax = Upper, fill = Grade, shape = '0'),
                           show.legend = FALSE) + 
            geom_line(data = wq.alt7.idx.subregion %>%
                          filter(Region != 'Cape York')) +
            geom_point(data = wq.alt7.idx.subregion %>%
                           filter(Region != 'Cape York'),
                       aes(fill = Grade, shape = '7'), size = 2 * point_mult) + 
            geom_linerange(data = wq.alt7.idx.subregion %>%
                               filter(Region != 'Cape York'),
                           aes(ymin = Lower, ymax = Upper, fill = Grade, shape = '7'),
                           show.legend = FALSE) + 
            scale_y_continuous('Water Qualtity Index',
                               limits = c(-1,1)) +
            scale_x_date('', limits = c(as.Date('2006-01-01'),
            ...
            scale_fill_manual('', breaks = c('A','B','C','D','E'),
                              ...
                              labels = c('Very Good','Good','Moderate','Poor','Very Poor'),
                              guide = FALSE) +
            scale_shape_manual('Option', values = c(21,24,23,22,25), guide = FALSE) +
            theme_mmp +
            theme(strip.background = element_blank(),
                  panel.margin.x = unit(1, 'line'))

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_subregion_new",
                               Plot = wq.comb.subregion.g1 + facet_grid(~Subregion,as.table=FALSE),
                               units = "in",
                               fig.width = 10, fig.height = 2, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional Index (new)", fig_name_suffix = "wq_worms_comb_subregion_new",
                           label_suffix = "_9_idx6bb2", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_subregion1_new",
                               Plot = wq.comb.subregion.g1 + facet_wrap(~Subregion,as.table=FALSE,nrow=1,scales='free'),
                               units = "in",
                               fig.width = 10, fig.height = 2, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional Index alt. (new)", fig_name_suffix = "wq_worms_comb_subregion1_new",
                           label_suffix = "_9_idx6bba2", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_subregion2_new",
                               Plot = wq.comb.subregion.g1 +
        ...
        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional Index alt.2 (new)", fig_name_suffix = "wq_worms_comb_subregion2_new",
                           label_suffix = "_9_idx6bbb", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 
    ...
    ## 6. Subregions No Cape York or Fitzroy (old)
    ...
    ## 6b. Subregions No Cape York or Fitzroy 
    ## ---- Subregions No Cape York or Fitzroy
    MMP_tryCatch(
    {
        load(file=paste0(DATA_PATH, '/indices/wq.historic.idx.RData'))
        ...
        wq.alt7.idx.subregion_noCY <- wq.alt7.idx.subregion %>%
            filter(!Subregion %in% c('Endeavour','Normanby', 'Pascoe', 'Stewart')) %>%
            droplevels()
        ...
        wq.comb.subregion.g1 <-
            ...
            geom_linerange(aes(ymin = Lower, ymax = Upper, fill = Grade,
                               shape = '0'),
                           show.legend = FALSE) + 
            geom_line(data = wq.alt7.idx.subregion_noCY) +
            geom_point(data = wq.alt7.idx.subregion_noCY,
                       aes(fill = Grade, shape = '7'), size = 2 * point_mult) +
            geom_linerange(data = wq.alt7.idx.subregion_noCY,
                           aes(ymin = Lower, ymax = Upper, fill = Grade,
                               shape = '7'),
                           show.legend = FALSE) + 
            scale_y_continuous('Water Qualtity Index',
        ...
        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_subregion_NoCapeYork_new",
                               Plot = wq.comb.subregion.g1 + facet_grid(~Subregion,as.table=FALSE),
                               units = "in",
                               fig.width = 15, fig.height = 2, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional Index (No CY, new)", fig_name_suffix = "wq_worms_comb_subregion_NoCapeYork_new",
                           label_suffix = "_9_idx7b", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 

        MMP__figure_export_dev(FIGURE_OUTPUT_PATH, fig_name_suffix = "wq_worms_comb_subregion1_NoCapeYork_new",
                               Plot = wq.comb.subregion.g1 + facet_wrap(~Subregion,as.table=TRUE,nrow=3,scales='free'),
                               units = "in",
                               fig.width = 7, fig.height = 4.5, pt.size = 10)

        MMP__figure_quarto(CURRENT_STAGE, "calculate indices", FIGURE_OUTPUT_PATH,
                           Section = "Subregional Index (No CY, new) alt.", fig_name_suffix = "wq_worms_comb_subregion1_NoCapeYork_new",
                           label_suffix = "_9_idx7ab", tabset_parent = "TABSET_9",
                           fig.caption = paste0("\nComparison of temporal trends in the water quality index for the historic and alt 7 formulations conditional on Subregion.\n")) 
        
    },
    LOG_FILE, item = CURRENT_ITEM, Category = 'Indices:', msg='Comparing indices (subregions, no Cape York, new).', return=TRUE)
    ## ----end
    ...
    MMP_tryCatch(
    {
        files <- paste0(FIGURE_OUTPUT_PATH,
                       c(
                           ...
                           "wq_worms_comb_region_new.pdf",
                           "wq_worms_comb_region_new_large.png",
                           "wq_worms_comb_region_NoFitzroy_new.pdf",
                           "wq_worms_comb_region_NoFitzroy_new_large.png"
                       )
                       )
